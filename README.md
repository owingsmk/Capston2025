...
**Harden Account Password Settings** – Sets minimum password length of 12, enables password complexity, and sets a maximum password age of 365 days for users.
**Disable Autorun** - Disables AutoRun on your system.
**Harden Windows Defender** - Enables all Windows firewall profiles & Windows Defender real-time protection.
**Tweak Registry Config** – Enables USB Write Protection on your system.

**Report Explanations:
Audit User Accounts** – Identifies all administrator accounts on your system and suggests remediation steps depending on the account.
**Check Firewall Status** – Identifies all Windows Firewall profiles and detects if any are disabled.
**Check Windows Updates** – Checks if automatic updates are enabled, identifies when the last successful update was installed.
...
**Review Password Policy** – Checks to see if all account passwords meet these standards and warns you if they do not. Standards: Minimum Password Length: 12, Password Complexity: Enabled, and Maximum Password Age: ≤ 365 days.
...
**Monitor Windows Event Logs** – Returns security related events from the last seven days and determines their severity.
